...
import { ImageAvatar, MainSection } from '@/components/ui/StyledComponents'
...
const getUserRepositories = async (id) => {
  const response = await getAllDataGet(`${GITHUB_API_BASE_URL}/users/${id}/repos?per_page=15`)
...
const page = async ({ params }) => {
  ...
  const repos = await getUserRepositories(id)
  return (
    <div style={{
      display: 'flex',
      ...
      <MainSection>
        <ImageAvatar src={user.avatar_url} width={200} height={200} alt="avatar" priority />
        <div >
          <div class="user-top">

            <h1 class="user-name" id="user-name">{user.name}</h1>
            <time class="body user-joined" id="user-joined-time"> {formatDate(user.created_at)} </time>
            <h3 class="user-username" id="user-username">@{user.name}</h3>
          </div>
          <p class="body user-bio" id="user-bio">  {user.bio}</p>
          <ul class="user-stats">
            <li>
              <h4 class="user-stat-title">Repositorios</h4>
              <h2 class="user-stat-value" id="user-repos">{user.public_repos}</h2>
            </li>
            <li>
              <h4 class="user-stat-title">Seguidores</h4>
              <h2 class="user-stat-value" id="user-followers">{user.followers}</h2>
            </li>
            <li>
              <h4 class="user-stat-title">Siguiendo</h4>
          ...
          <section class="user-links">
            <div class="user-link-container" id="user-location">
              ...
              <span class="body user-link">{user.location ? user.location : 'No disponible'}</span>
            </div>
            <div class="user-link-container" id="user-twitter">
              ...
              {
                user.twitter_usernam ? <Link href={` https://twitter.com/${user.twitter_username}`} target="_blank" >{`@${user.twitter_username}`}   </Link>
                  : 'No disponible'
              }
            ...
            <div class="user-link-container" id="user-website">
              ...
              {
                user.blog ?
                  <Link href={user.blog} target="_blank"  > {user.blog}</Link>
                  : 'No disponible'
              }
            </div>
            <div class="user-link-container" id="user-organization">
              ...
              {
                user.company ?
                  <Link href={` ${user.company}`} target="_blank" >{user.company}</Link>
                  : 'No disponible'
              }
            </div>
          </section>
...
export const parsedate = (dateparse) => {
