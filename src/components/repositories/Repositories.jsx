...
const Repositories = ({ userRepositories }) => {
  ...
  useEffect(() => {
    const getRepositories = async () => {
      setIsLoading(true)
      const { repositories, pagination, error, } = await getUserRepositories(userRepositories, tonextPage,)
  ...
  const getPagination = async (pagination) => {
    ...
    if (pageLink.find(link => link.includes('last'))) {
      const includeLast = pageLink.find(link => link.includes('last')).split(';')[0]
    ...
    if (pageLink.find(link => link.includes('next'))) {
      const includeNext = pageLink.find(link => link.includes('next')).split(';')[0]
  ...
  return (
    <div className="">
      ...
      <h3 className="my-4 text-2xl font-semibold text-center">Repositorios Públicos</h3>
      <InfiniteScroll
